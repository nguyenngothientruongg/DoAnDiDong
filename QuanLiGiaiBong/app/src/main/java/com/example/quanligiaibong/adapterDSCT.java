...
public class adapterDSCT extends ArrayAdapter {
    Context context;
    int resource;
    List<ClassCT> data;
    public adapterDSCT(@NonNull Context context, int resource, List<ClassCT> data) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        TextView tvtuoiCT = convertView.findViewById(R.id.tvtuoiCT);
        TextView tvqtct = convertView.findViewById(R.id.tvqtCT);
        ImageView ivdbct = convertView.findViewById(R.id.ivdbct);
        ImageView ivgt = convertView.findViewById(R.id.ivGioiTinh);
        ClassCT dsCT = data.get(position);
        tvmaCT.setText(dsCT.getMaCT());
        tvtenCT.setText(dsCT.getTenCT());
        tvtuoiCT.setText(dsCT.getNamCT());
        tvqtct.setText(dsCT.getGtCT());
        LayAnhDB(dsCT.getDbCT(), ivdbct);
        if (dsCT.getGtCT().equals("Nam"))
        {
            ivgt.setImageResource(R.drawable.baseline_male_24);
        }
        else
        {
            ivgt.setImageResource(R.drawable.baseline_female_24);
        }
        return convertView;
    }
